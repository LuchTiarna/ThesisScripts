...
setsOfPredictors <- list(
  ...
  data.frame(predictor=c(7.09,  7.99,  8.68,  9.02,  9.71, 10.64), observations=240,   id=1, distr=1),
  data.frame(predictor=c(4.72,  7.09,  7.99,  9.71, 10.64, 13.21), observations=240,   id=2, distr=2),
  data.frame(predictor=c(7.09,  8.34, 10.64, 11.72, 13.21, 15.76), observations=240,   id=3, distr=3),
  data.frame(predictor=c(4.72,  6.07,  7.09,  7.99,  8.85,  9.71), observations=240,   id=4, distr=4),
  data.frame(predictor=c(4.37,  5.83,  6.90, 10.64, 12.38, 16.64), observations=240,   id=5, distr=5),
  data.frame(predictor=c(7.09,  7.99,  8.85,  9.71, 10.64, 16.64), observations=240,   id=6, distr=6),
  data.frame(predictor=c(7.46,  8.34,  9.19, 11.72, 13.21, 15.76), observations=240,   id=7, distr=7)

)

#Data generating functions + inner parameters a,b
functions <- list(
  list("exponential", "polynom", c(8.85/((log(2)^(1/3))), 3)),
...
###saving data to file
write.csv(generated_data.frame, "gen_funcComp.data.csv")
cat("End of computation at:", " ", date(), "\n", sep="")

cat("Number of generated instances:", " ", nrow(generated_data.frame)/6, "\n", sep="")
